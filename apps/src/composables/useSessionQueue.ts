...
export function useSessionQueue(pageSize = 5) {
  ...
  // N-01：仅在新会话入队时（长度增大）才重置分页，避免 ACCEPTED/CLOSED 移除条目时打断用户翻页
  watch(
    () => queue.value.length,
    (newLen, oldLen) => {
  ...
  async function loadQueue() {
    try {
      ...
      // I-15：加载失败时记录日志，不静默吞掉，便于排查接口故障
      console.error('[useSessionQueue] loadQueue 失败:', error);
    }
  }
