...
const domainForm = ref<Record<string, any>>({});
const intentForm = ref<Record<string, any>>({});
const slotForm = ref<Record<string, any>>({});
const bindingForm = ref<Record<string, any>>({});
...
const bindingColumns = [
  { title: '工具', key: 'toolId' },
  { title: '模式', dataIndex: 'executionMode', key: 'executionMode', width: 120 },
  { title: '顺序', dataIndex: 'executionOrder', key: 'executionOrder', width: 70 },
...
async function saveDomain() {
  ...
  if (editingDomain.value?.id) {
    await updateDomainApi(editingDomain.value.id, domainForm.value as DomainDTO);
...
async function saveIntent() {
  if (!intentForm.value.code || !intentForm.value.name || !intentForm.value.description) {
    message.error('code、名称、描述为必填');
    return;
  }
  const data = { ...intentForm.value, domainId: selectedDomainId.value! } as IntentDTO;
...
async function saveSlot() {
  ...
  const data = { ...slotForm.value, intentId: selectedIntentId.value! } as SlotDTO;
  if (editingSlot.value?.id) {
    await updateSlotApi(editingSlot.value.id, data);
  } else {
    await createSlotApi(data);
  }
...
async function saveBinding() {
  ...
  const data = { ...bindingForm.value, intentId: selectedIntentId.value! } as BindingDTO;
  ...
  <div class="dit-domains-page">
    ...
    <div class="left-panel">
      <div class="panel-header">
        ...
        <Button type="primary" size="small" @click="openCreateDomain">+ 新建</Button>
      </div>
      <div
        v-for="d in domains"
        :key="d.id"
    ...
    <div class="right-panel">
      <template v-if="selectedDomainId">
        ...
        <div class="intent-list">
          ...
          >
            <span>{{ i.name }} <small style="color:#999">({{ i.code }})</small></span>
            <Space size="small">
        ...
        <Tabs v-model:activeKey="activeTab" style="margin-top: 16px">
          ...
          <TabPane key="basic" tab="基本信息">
            <Descriptions :column="2" bordered size="small">
              <DescriptionsItem label="意图码">{{ selectedIntent.code }}</DescriptionsItem>
              <DescriptionsItem label="名称">{{ selectedIntent.name }}</DescriptionsItem>
              <DescriptionsItem label="自动转人工">
                <Tag :color="selectedIntent.autoTransfer ? 'orange' : 'default'">
              ...
              <DescriptionsItem label="描述" :span="2">{{ selectedIntent.description }}</DescriptionsItem>
          ...
          <TabPane key="slots" tab="槽位配置">
            <Button size="small" style="margin-bottom: 8px" @click="openCreateSlot">
              + 添加槽位
            </Button>
            <Table
              :data-source="slots"
          ...
          <TabPane key="tools" tab="工具绑定">
            <Button size="small" style="margin-bottom: 8px" @click="openCreateBinding">
              + 绑定工具
            </Button>
            <Table
            ...
            >
              <template #bodyCell="{ column, record }">
                ...
                <template v-if="column.key === 'executionMode'">
                  <Tag :color="record.executionMode === 'REQUIRED' ? 'red' : 'blue'">
    ...
    >
      <Form layout="vertical">
        <FormItem label="领域码 *"><Input v-model:value="domainForm.code" placeholder="如：ecommerce" /></FormItem>
        <FormItem label="名称 *"><Input v-model:value="domainForm.name" placeholder="如：电商客服" /></FormItem>
        <FormItem label="描述"><Input v-model:value="domainForm.description" /></FormItem>
        <FormItem label="System Prompt 追加"><Textarea v-model:value="domainForm.systemPromptAddon" :rows="4" /></FormItem>
        <FormItem label="启用"><Switch v-model:checked="domainForm.enabled" /></FormItem>
      </Form>
      <template #footer>
        <Button @click="domainDrawerVisible = false">取消</Button>
        <Button type="primary" style="margin-left: 8px" @click="saveDomain">保存</Button>
      </template>
    </Drawer>
    ...
    >
      <Form layout="vertical">
        <FormItem label="意图码 *"><Input v-model:value="intentForm.code" placeholder="如：query_order" /></FormItem>
        <FormItem label="名称 *"><Input v-model:value="intentForm.name" placeholder="如：查询订单" /></FormItem>
        <FormItem label="描述 *"><Textarea v-model:value="intentForm.description" :rows="2" /></FormItem>
        <FormItem label="示例句子 (JSON数组)">
          <Textarea v-model:value="intentForm.exampleQueries" :rows="3" placeholder='["帮我查订单","我的包裹到哪了"]' />
        </FormItem>
        <FormItem label="自动转人工"><Switch v-model:checked="intentForm.autoTransfer" /></FormItem>
        <FormItem label="跳过RAG"><Switch v-model:checked="intentForm.skipRag" /></FormItem>
        <FormItem label="工具失败兜底回复"><Input v-model:value="intentForm.fallbackReply" /></FormItem>
      </Form>
      <template #footer>
        <Button @click="intentDrawerVisible = false">取消</Button>
        <Button type="primary" style="margin-left: 8px" @click="saveIntent">保存</Button>
      </template>
    </Drawer>
    ...
    >
      <Form layout="vertical">
        <FormItem label="槽位名 *"><Input v-model:value="slotForm.slotName" placeholder="如：order_id" /></FormItem>
        <FormItem label="类型">
        ...
        <FormItem label="说明 *"><Input v-model:value="slotForm.description" /></FormItem>
        <FormItem label="必填"><Switch v-model:checked="slotForm.required" /></FormItem>
        <FormItem label="解析策略 (JSON数组)">
          <Input v-model:value="slotForm.resolveStrategy" placeholder='["EXTRACT","SESSION","DISCOVER","ASK_USER"]' />
        </FormItem>
        <FormItem label="发现工具码"><Input v-model:value="slotForm.discoverToolCode" /></FormItem>
        <FormItem label="询问话术"><Input v-model:value="slotForm.askUserPrompt" /></FormItem>
      </Form>
      <template #footer>
        <Button @click="slotDrawerVisible = false">取消</Button>
        <Button type="primary" style="margin-left: 8px" @click="saveSlot">保存</Button>
      </template>
    </Drawer>
    ...
    <Drawer v-model:open="bindingDrawerVisible" title="绑定工具" width="420">
      <Form layout="vertical">
        <FormItem label="工具 *">
          <Select v-model:value="bindingForm.toolId" style="width: 100%" placeholder="请选择工具">
        ...
        <FormItem label="执行顺序">
          <InputNumber v-model:value="bindingForm.executionOrder" :min="0" style="width: 100%" />
        </FormItem>
      </Form>
      <template #footer>
        <Button @click="bindingDrawerVisible = false">取消</Button>
        <Button type="primary" style="margin-left: 8px" @click="saveBinding">绑定</Button>
      </template>
    </Drawer>
  </div>
</template>
...
.domain-item:hover,
.domain-item.active { background: #e6f7ff; }
...
.intent-item:hover,
.intent-item.active { background: #f0f5ff; }
</style>
